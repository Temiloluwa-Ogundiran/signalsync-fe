...
import { useCreateManualTrade } from "../hooks/use-manual-trade";
...
export function AddTradeModal() {
  ...
  useEffect(() => {
    if (addTradeModalOpen) {
      ...
      setSl("");
      setTp("");
      setMissedDate(prefilledAddTradeDate || "");

      const now = new Date();
      if (prefilledAddTradeDate) {
        // e.g. "2026-05-28" -> prefill open and close date to that local date at 9:00 and 10:00
        const pDate = new Date(prefilledAddTradeDate);
        pDate.setHours(9, 0, 0, 0);
        setOpenedAt(formatToDatetimeLocal(pDate));

        const pDateClose = new Date(prefilledAddTradeDate);
        pDateClose.setHours(10, 0, 0, 0);
        setClosedAt(formatToDatetimeLocal(pDateClose));
      } else {
        setOpenedAt(formatToDatetimeLocal(now));
        const oneHourLater = new Date(now.getTime() + 60 * 60 * 1000);
        setClosedAt(formatToDatetimeLocal(oneHourLater));
      }
    }
  }, [addTradeModalOpen, prefilledAddTradeDate]);
  ...
  const getPayload = (): Partial<ManualTradeCreatePayload> => {
    ...
    if (isMissed) {
      ...
      } else if (prefilledAddTradeDate) {
        targetDateIso = new Date(
          `${prefilledAddTradeDate}T09:00:00`,
        ).toISOString();
      }
    }
  ...
  const handleSubmit = async () => {
    const payload = getPayload() as ManualTradeCreatePayload;
    const currentErrors = validateManualTrade(payload);
    ...
    try {
      const createdTrade = await createManualTrade.mutateAsync(payload);
      ...
      const msg =
        err?.message || "An error occurred while saving the manual trade.";
      setErrors({ root: msg });
      toast.error("Failed to save trade", { description: msg });
    }
  };
      ...
      <DialogContent className="max-w-2xl border border-border-primary bg-card-bg p-0 overflow-hidden rounded-2xl shadow-2xl">
              ...
              {!isMissed ? (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0 bg-card-bg border border-border-primary rounded-xl" align="start">
                      <Calendar
                        mode="single"
                        selected={parsedDate}
              ...
              <div className="rounded-xl border border-border-secondary bg-bg-tertiary overflow-hidden">
                <div className="px-4 py-3 flex items-center justify-between border-b border-border-secondary bg-accent/10 text-accent">
                  <span className="text-xs font-bold">
                    {isMissed ? "Missed Trade Setup" : "Manual Trade Receipt"}
                  </span>
                      ...
                      <div>
                        ...
                        >
                          {Number(netProfit) >= 0
                            ...
                            : `-$${Math.abs(Number(netProfit)).toFixed(2)}`}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-text-secondary font-semibold">
        ...
        <div className="bg-bg-tertiary px-6 py-4 border-t border-border-secondary flex items-center justify-between">
          <div>
            {step > 1 ? (
              <button
                type="button"
                onClick={handleBack}
                disabled={createManualTrade.isPending}
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setAddTradeModalOpen(false)}
              disabled={createManualTrade.isPending}
