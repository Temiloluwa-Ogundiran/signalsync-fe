...
/**
 * Subtle, dismissible notice shown while the active account is the seeded demo.
 * Offers a one-click "Clear demo data" that removes the demo account (the
 * backend cascade-deletes its trades/journals), and connecting a real account
 * also clears it server-side.
 */
export function DemoDataBanner({ isDemo }: { isDemo: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-ai-soft-bg px-4 py-3 ring-1 ring-inset ring-ai-soft-border">
      <HugeiconsIcon
      ...
      <button
        ...
        onClick={dismiss}
        aria-label="Dismiss"
        className="shrink-0 rounded-md p-1 text-text-tertiary transition-colors hover:text-text-secondary cursor-pointer"
      >
