...
export function useTelegramConnections() {
  const { token, enabled } = useCopyTradingAuth();
  return useQuery({
    queryKey: COPY_TRADING_KEYS.connections(),
    queryFn: () => copyTradingApi.listConnections(token),
    enabled,
...
export function useTelegramSources() {
  const { token, enabled } = useCopyTradingAuth();
  return useQuery({
    queryKey: COPY_TRADING_KEYS.sources(),
    queryFn: () => copyTradingApi.listSources(token),
    enabled,
    refetchInterval: 5000,
  });
}
...
export function useCopyTradingActions() {
  ...
  return {
    startPhone: useMutation({
      mutationFn: (phone: string) =>
        copyTradingApi.startPhoneAuth(phone, token),
    }),
    startQr: useMutation({
      mutationFn: () => copyTradingApi.startQrAuth(token),
    }),
    getAuth: (authId: string) => copyTradingApi.getAuth(authId, token),
    submitCode: (authId: string, code: string) =>
      copyTradingApi.submitCode(authId, code, token),
    submitPassword: (authId: string, password: string) =>
      copyTradingApi.submitPassword(authId, password, token),
    disconnect: useMutation({
      mutationFn: (id: string) => copyTradingApi.disconnect(id, token),
      onSuccess: refresh,
    }),
    pauseConnection: useMutation({
      mutationFn: ({ id, paused }: { id: string; paused: boolean }) =>
        copyTradingApi.pauseConnection(id, paused, token),
      onSuccess: refresh,
    }),
    createSource: useMutation({
      mutationFn: (
        payload: Parameters<typeof copyTradingApi.createSource>[0],
      ) => copyTradingApi.createSource(payload, token),
      onSuccess: refresh,
    }),
    pauseSource: useMutation({
      mutationFn: ({ id, paused }: { id: string; paused: boolean }) =>
        copyTradingApi.pauseSource(id, paused, token),
      onSuccess: refresh,
    }),
    deleteSource: useMutation({
      mutationFn: (id: string) => copyTradingApi.deleteSource(id, token),
      onSuccess: refresh,
    }),
    createCopyConnection: useMutation({
      mutationFn: (
        payload: Parameters<typeof copyTradingApi.createCopyConnection>[0],
    ...
    deleteCopyConnection: useMutation({
      mutationFn: (id: string) =>
        copyTradingApi.deleteCopyConnection(id, token),
      onSuccess: refresh,
    }),
    revealRaw: (eventId: string) =>
      copyTradingApi.revealActivityRaw(eventId, token),
    createRoute: useMutation({
      mutationFn: (payload: Parameters<typeof copyTradingApi.createRoute>[0]) =>
        copyTradingApi.createRoute(payload, token),
      onSuccess: refresh,
    }),
    updateRoute: useMutation({
      mutationFn: ({
        ...
        id: string;
        payload: Partial<Parameters<typeof copyTradingApi.createRoute>[0]>;
      }) => copyTradingApi.updateRoute(id, payload, token),
      onSuccess: refresh,
    }),
    routeAction: useMutation({
      mutationFn: ({
        id,
        action,
      }: {
        ...
        action === "activate"
          ? copyTradingApi.activateRoute(id, token)
          : action === "pause"
            ? copyTradingApi.pauseRoute(id, token)
            : copyTradingApi.resumeRoute(id, token),
      onSuccess: refresh,
    }),
